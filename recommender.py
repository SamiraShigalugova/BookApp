# recommender.py - исправленная версия для работы со строковыми book_id
...
class EmbeddingModel:
    """
    Управление семантическими эмбеддингами книг на основе описаний.
    """
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        self.model = SentenceTransformer(model_name)
        self.embeddings: Dict[str, np.ndarray] = {}  # ключ – строковый book_id
    ...
    def build_embeddings(self, books: List[Dict[str, Any]]):
        ...
        id (str), title, author, genre, tags, description.
        """
        texts = []
        valid_indices = []
        ...
        for idx, vec in zip(valid_indices, vectors):
            ...
            self.embeddings[book_id] = vec

    def get(self, book_id: str) -> Optional[np.ndarray]:
        """Возвращает эмбеддинг книги, если он есть."""
...
class BookRecommender:
    ...
    - семантическую близость (эмбеддинги описаний)
    ...
    def build(self, interactions: List[Dict], books: List[Dict]):
        ...
        # Строим эмбеддинги книг (семантические векторы)
    ...
    def recommend(
        ...
        # Строим семантический профиль (средний эмбеддинг оценённых книг)
        ...
        for book in candidate_books:
            ...
            # Семантический скор (косинус эмбеддингов) -> преобразуем в [0..1]
            semantic = self._semantic_score(book_id, user_embedding)
            semantic_norm = (semantic + 1) / 2   # из [-1,1] в [0,1]

            # Базовый скор для холодного старта (мало оценок)
            if n_ratings < 3:
                score = content * 0.6 + semantic_norm * 0.4
            else:
                # Коллаборативный скор (item-based) -> нормируем делением на 5
                collab = self._collaborative_score(user_id, book_id) / 5.0
                score = content * 0.35 + semantic_norm * 0.45 + collab * 0.20
    ...
    def _build_user_embedding(self, positive_ratings: List[Dict]) -> Optional[np.ndarray]:
        """Строит средний эмбеддинг пользователя на основе оценённых книг, взвешенный по рейтингу."""
        vectors = []
        weights = []
        for inter in positive_ratings:
            book_id = inter["book_id"]
        ...
        try:
            ...
            print(f"⚠️ Ошибка построения пользовательского эмбеддинга: {e}")
            return None

    def _semantic_score(self, book_id: str, user_embedding: Optional[np.ndarray]) -> float:
        """Косинусное сходство между эмбеддингом книги и пользовательским профилем."""
        if user_embedding is None:
            return 0.0
        book_vec = self.embedding_model.get(book_id)
        if book_vec is None:
            return 0.0
        cos_sim = np.dot(book_vec, user_embedding) / (np.linalg.norm(book_vec) * np.linalg.norm(user_embedding) + 1e-9)
